...
import v3.residentialPropertyDisposals.retrieveNonPpd.def2.model.request.Def2_RetrieveNonPpdResidentialPropertyRequestData
...
@Singleton
class Def2_RetrieveNonPpdResidentialPropertyCgtValidator @Inject() (nino: String, taxYear: String)
  ...
  override def validate: Validated[Seq[MtdError], RetrieveNonPpdResidentialPropertyCgtRequestData] =
    ResolveNino(nino).map(validNino => Def2_RetrieveNonPpdResidentialPropertyRequestData(validNino, TaxYear.fromMtd(taxYear)))
