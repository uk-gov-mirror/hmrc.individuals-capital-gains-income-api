...
import v3.residentialPropertyDisposals.retrieveNonPpd.RetrieveNonPpdResidentialPropertyCgtSchema.*

class RetrieveNonPpdResidentialPropertyCgtSchemaSpec extends UnitSpec with ScalaCheckDrivenPropertyChecks with TaxYearPropertyCheckSupport {

  "schema lookup" when {
    "a valid tax year is supplied" must {
      "use Def1 schema for tax years between 2020-21 and 2024-25" in {
        forTaxYearsInRange(TaxYear.fromMtd("2020-21"), TaxYear.fromMtd("2024-25")) { taxYear =>
    ...
    "handle errors" when {
      "an invalid tax year is supplied" must {
        "disallow tax years prior to 2019-20 and return RuleTaxYearNotSupportedError" in {
