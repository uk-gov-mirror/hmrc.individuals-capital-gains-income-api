...
class RetrieveNonPpdResidentialPropertyCgtResponseSpec extends UnitSpec {
  ...
  val model: Def1_RetrieveNonPpdResidentialPropertyCgtResponse =
    Def1_RetrieveNonPpdResidentialPropertyCgtResponse(
      Some(customerAddedDisposals)
    )

  "RetrieveNonPpdResidentialPropertyCgtResponse" when {
    "Reads" should {
      "return a valid object" when {
        "a valid json is supplied" in {
          downstreamJson.as[Def1_RetrieveNonPpdResidentialPropertyCgtResponse] shouldBe model
        }
      }
    }
